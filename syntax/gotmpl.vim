...
syn keyword     gotmplControl     contained   if else end range with template
syn keyword     gotmplFunctions   contained   and html index js len not or print printf println urlquery
syn match       gotmplVariable    contained   /\$[^ ]*\>/

hi def link     gotmplControl        Keyword
hi def link     gotmplFunctions      Keyword

syn region gotmplAction start="{{" end="}}" contains=@gotmplLiteral,gotmplControl,gotmplFunctions,gotmplVariable display
